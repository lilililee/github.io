...
//***********************************鼠标事件*****************************************
//注意！！！用$.addEvent绑定事件，在IE8-中this不是指定当前绑定事件的节点，callback函数中有this会导致出错
//可以按需求来调整$.addEvent的函数内容（在$.addEvent中已去除attachEvent方法）
//单击
...
//内容改变，在失去焦点时才会检测
//text,textarea,下拉菜单,单选按钮,复选按钮  可以触发change事件
...
Anything.prototype.show = function(time, callback){
	...
	var i,
		j,
		len1 = this.length,
		attr_list = ['opacity','width','height','padding-top','padding-right','padding-bottom','padding-left',
...
Anything.prototype.fadeIn = function(time, callback){
	...
	for( i = 0; i < len1; i++){
		...
		$(this[i]).animate(obj_attr, time, callback, 'fadeIn', function(){
			...
			//alert($.getStyle(this,'opacity'));   //在IE678得到0！！！！！！！！！后面修改了$.getStyle函数解决	
			this.style['display'] = this.old_style['display'];
		});
	}
...
Anything.prototype.fadeTo = function(time, opacity, callback){
	//1. time和opacity是必选参数
	if(typeof time != 'number' || typeof opacity != 'number') $.errorArgs();
...
Anything.prototype.slideDown = function(time, callback){
	...
	var i,
		j,
		len1 = this.length;
		attr_list = ['height','padding-top','padding-bottom','margin-top','margin-bottom','display','overflow'];
...
Anything.prototype.animate = function(obj_attr, time, callback, type, start_fn){
	...
	for( i = 0; i < len1; i++ ){
		...
		//3. 启动第一次动画，后续动画将在$.doAnimate中回调执行
...
//动画执行函数
//每执行一次$.doAnimate表示完成了当前节点队列中的一个动画
//延时也看作一个动画
//obj_attr中必须均为带数字的属性，display，position等不行
$.doAnimate = function(node, obj_attr, time, callback, delay, type, start_fn){
	...
	var n = Math.floor(time/20),				//20ms移动一次，计算此次动画的移动次数
		count = 0,								//当前已执行的动画次数
		all_attr = [],							//当前节点需要执行动画的属性
		all_attr_end = [],						//当前节点属性最终值
		all_attr_start = [],					//当前节点属性初始值
		all_attr_now = [],						//当前节点属性当前值
		all_attr_dis = [],						//当前节点属性每次动画的增加值
	...
	for( attr in obj_attr){
		if(attr == 'scrollTop'){
			cur_start = $(node).scrollTop();	//获取当前节点的滚轮值
		} else {
			cur_start = parseFloat($.getStyle(node ,attr));	//当前属性的初始值，以数值形式保存，去除单位，方便计算
		}
		
		cur_end = parseFloat(obj_attr[attr]);			//当前属性的结束值，以数值形式保存，去除单位，方便计算
	...
	node.animate_args.interval_id = setInterval(function(){
		...
		if( count<n-1 ){	//console.log(22222)
			for( i = 0; i < all_attr.length; i++ ){		//遍历需要执行动画的属性						
...
//AJAX封装
$.ajax = function(args){
	//1. 设置参数默认值，data默认值在dealData中设置
	if( args.type === undefined ) args.type = 'get';
	if( args.async  === undefined ) args.async  = true; 

	//2. 生成XMLHttpRequest对象
	var xhr = createXHR(),
		post_data = null;
	...
	if( args.data instanceof Array ){
		if( args.type === 'get' ){
			...
			args.url += dealDate(args.data);
			//alert(args.url);
		} else if (args.type === 'post'){
			post_data = dealDate(args.data);
			//alert(post_data);
		} else {
			throw new Error('type参数必须为get或post');
		}
	} else if ( args.data !== undefined ){
		throw new Error('data参数必须为数组形式');	//data只能为数组或undefined
	}
	

	//4. 如果为异步，提前绑定onreadystatechange事件
	if( args.async === true ){
...
/*
$.ajax({
		url : 'http://127.0.0.1/ajaxTest/data.php?rand='+Math.random(),
		type : 'post',
		data : [{
			name : 'lee',
			age : '23'
...
//高级程序设计P219
$.isHostMethod = function(obj, property){
...
$.pushElementsToAnything = function(anything, arr){
...
$.elementIsMatchSelect = function(node, select){
	var is_match = false;
...
//获取元素节点的默认display
$.getDefaultDisplay = function(node){	
...
$.contains = function(parent_node,child_node){
...
//或者可以像上面$.documentReady中使用arguments.calee$.removeEvent
...
//3. 获取鼠标在整个屏幕的位置
$.getScreenPosition = function(event){
	var e = event || window.event;
	return {
		left : e.screenX,
		top : e.screenY
